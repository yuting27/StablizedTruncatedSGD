...
### Main algorithm with logistic loss
### Author: Yuting Ma
### Date: 04/21/2016
...
stabilizedTruncatedSGD_logistic <- function(X, y, X_test, y_test, seed, par_list){
  ...
  b_rej <- rep(0, p)
  while(min(tt_list) < maxIter | delta < optTol){
    s <- 1
    b_Sel <- rep(0, ncol(X_new))
    b_nz_sum <- rep(0, ncol(X_new))
    while(s <= nK){
      
      burst_logistic_trunc <- function(h){
        ptm <- proc.time()
        v_00 <- v_list[[h]]
        v <- v_00
        ...
        v_new <- normalizer(as.vector(trunc$v_new))
      ...
      } # end of function burst_logistic_trunc
      
      sgd_chain <- mclapply(1:n_chain, burst_logistic_trunc, mc.cores=n_chain)
      ...
      mark_list <- rbind(mark_list, tt_list)
  ...
  return(list(w=v, train_err = train_err_tbl, test_err=test_err_tbl,
